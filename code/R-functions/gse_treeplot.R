...
##' @rdname gse_treeplot
##' @importFrom https://github.com/YuLab-SMU/enrichplot/blob/master/R/treeplot.R
...
##'
gse_treeplot <- function(x,
                                  showCategory = 30,
                                  color = "p.adjust",
                                  color_palette = hcl.colors(10),
                                  nWords = 4,                   # removed
                                  nCluster = 5,                 # removed
                                  cex_category = 1,
                                  label_format = NULL,
    ...
    p <- group_tree(hc = hc, clus = clus, d = d, offset_tiplab = offset_tiplab,
        ...
        color = color, color_palette = color_palette)
    ...
    p <- p + scale_size_continuous(name = "number of genes",
...
group_tree <- function(hc, clus, d, offset_tiplab, nWords,
    ...
    if (add_tippoint) {
        p <- p + ggnewscale::new_scale_colour() +
            geom_tippoint(aes(color = color, size = count)) +
            scale_colour_gradientn(colours = color_palette, name = color, guide = guide_colorbar(reverse = TRUE))
            #scale_colour_continuous(low="red", high="blue", name = color, guide = guide_colorbar(reverse = TRUE))
    }
    ## add tiplab
...
##' Utilities
##'
##' @importFrom https://github.com/YuLab-SMU/enrichplot/blob/master/R/utilities.R
