---
title: "Differential gene expression of BR16 dox treated samples"
...
params:
  date: '`r format(Sys.Date(), "%B %d, %Y")`'
  data_dir: ./output/rnaseq/br16_bulk
  output_dir: ./output/rnaseq/br16_bulk/dge
  se_file: se_gene.rds
  min_counts: 3
...
use_rows <- filterByExpr(se, group=se$condition, min.count = 1, min.prop = 0.3)
se <- se[use_rows,]
...
comp_list <- list(
  `p27-wt-96` = list(
    p27 = x %>% 
      filter(condition == 'p27_96h') %>% 
      pull(sample_alias) %>% 
      sort,
    wt = x %>% 
      filter(condition == 'WT_96h') %>% 
      pull(sample_alias) %>% 
      sort
  )
)
...
comp_list_description <- data.frame(
  ...
  comparison_long = c('p27--over--WT at 96h')
)
